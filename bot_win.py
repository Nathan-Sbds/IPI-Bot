...
async def on_interaction(ctx):
    if ctx.channel_id is None:
        await ctx.response.send("Le bot ne prend pas en charge les messages privés.")
...
async def on_ready():
    ...
    while not client.is_closed():
        now = datetime.now()
        next_execution_time_5h = datetime(now.year, now.month, now.day, 23, 32, 0)
...
async def all_agenda_week_print():
    ...
    for guild in client.guilds:
        if guild.id != 811572433865277440:
            members_with_role = []
    ...
    for member in members_with_role:
        try:
            ...
            if (str(member.id) in LoginPromoJson) and channel.permissions_for(
                ...
                cursor.execute(
                    "SELECT id FROM emploi_du_temps WHERE image_data = ? AND channel_id = ?",
                    (
                        image_data,
                        channel.id,
                    ),
                ...
                if existing_row:
                    ...
                    cursor.execute(
                        "DELETE FROM emploi_du_temps WHERE discord_id = ? AND channel_id = ?",
                        (
                            member.id,
                            channel.id,
                        ),
